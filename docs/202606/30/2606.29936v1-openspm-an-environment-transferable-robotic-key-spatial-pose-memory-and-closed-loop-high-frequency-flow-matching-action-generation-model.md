...
tldr: "开放环境桌面机器人操作需要语义理解、几何姿态估计和高频动作生成。OpenSPM通过语义3D感知和Kalman滤波跟踪连续6D姿态，从人类演示中提取关键空间姿态作为可迁移的空间持久记忆。推理时根据自然语言指令检索记忆，利用SE(3)变换转移姿态，并通过轻量条件流匹配模型生成高频动作块，结合实时状态反馈和终末残差校正抑制误差累积。在LIBERO-GOAL任务上达到85.6%成功率、1033.3Hz等效控制频率，验证了结构化空间记忆和闭环校正的有效性。"
source: arxiv
...
motivation: 端到端VLA模型缺乏几何约束且训练昂贵，需要桥接高层语义与低层物理执行。
method: 提出OpenSPM框架，包含语义3D感知与Kalman滤波的6D姿态跟踪、从演示中提取可迁移空间持久记忆，以及条件流匹配模型生成高频动作块。
result: "在10个LIBERO-GOAL任务上成功率达85.6%，等效控制频率1033.3Hz，推理算力需求低。"
conclusion: 结构化空间持久记忆和闭环残差校正对于可靠高频机器人操作至关重要。
---

## 摘要
开放环境下的桌面机器人操作要求系统具备语义理解、精确几何位姿估计和高频动作生成能力。虽然端到端的视觉-语言-动作（VLA）模型在语义泛化方面表现出色，但它们往往缺乏细粒度任务所需的显式几何约束，并且需要昂贵的训练。为了弥合高层语义与底层物理执行之间的鸿沟，我们提出了OpenSPM，一种开放环境空间持久记忆框架，由空间位姿记忆和流匹配动作生成模型组成。OpenSPM首先利用语义条件化的3D感知和卡尔曼滤波来跟踪连续的6D位姿。然后，它从人类演示中提取关键空间位姿，并将其保留为可迁移的、以物体为中心的空间持久记忆条目。在推理过程中，OpenSPM根据自然语言指令检索相关记忆条目，使用SE(3)变换将空间位姿迁移到新场景，并通过轻量级条件流匹配模型生成高频动作块。结合实时本体感受状态反馈和终端残差校正，该系统有效抑制了轨迹误差累积。在十个LIBERO-GOAL任务上的评估中，OpenSPM实现了85.6%的成功率和等效1033.3 Hz的控制频率，同时只需最小的推理AI计算能力。大量消融实验表明，结构化的空间持久记忆和闭环残差校正在可靠、高频的机器人操作中起着关键作用。
