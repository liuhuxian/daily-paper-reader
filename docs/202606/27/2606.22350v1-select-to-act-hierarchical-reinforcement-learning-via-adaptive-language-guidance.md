...
title_zh: 选择即行动：基于自适应语言指导的分层强化学习
...
tldr: 强化学习在复杂环境中样本效率低下，现有方法将自然语言指令作为单一条件输入，忽略了指令在不同阶段的指导作用。本文提出分层强化学习框架HRLLI，将指令分解为片段，利用高层语义策略动态选择当前状态最相关的指令片段，引导低层策略执行动作。在RTFM基准上，HRLLI显著优于现有指令条件基线，验证了自适应指令选择对提升RL有效性的关键作用。
source: arxiv
...
motivation: 现有方法将完整指令作为单一条件，忽视其阶段性，导致复杂环境下样本效率低。
method: 提出HRLLI，将指令分解为片段，高层策略选择当前最相关片段，低层策略据此执行动作。
result: 在RTFM基准上，HRLLI持续优于强指令条件基线。
conclusion: 显式建模自适应指令选择显著提升了强化学习有效性。
---

## 摘要
强化学习（RL）已广泛应用于序列决策，但由于与环境交互的成本高昂，通常面临样本效率低下的问题。近期少数研究开始探索利用自然语言指令中的外部知识来提升RL效率。然而，现有方法通常将整个指令作为单一条件输入，未能考虑语言指导的阶段依赖性，尤其是在复杂环境中。本文提出基于语言指令的分层强化学习（HRLLI），这是一种显式将自然语言指令建模为决策过程中动态可选的语义指导的分层RL框架。HRLLI将指令分解为一组分段指导元素，每个指令片段可能在与环境交互的不同阶段变得相关。进而形成一种新颖的分层RL策略结构，遵循“选择即行动”范式：高层语义策略作为指导选择器，选择与当前状态最相关的指令片段来指导底层智能体的决策，而底层策略则基于所选指导执行环境动作。两层策略同时学习，以最大化与环境交互带来的增强期望回报。这种设计使得智能体能够在交互过程中自适应地将语言指令落地为阶段特定的决策。在指令密集型RTFM基准上的实验表明，HRLLI持续优于强指令条件RL基线，证明显式建模自适应指令选择显著提升了RL的有效性。
