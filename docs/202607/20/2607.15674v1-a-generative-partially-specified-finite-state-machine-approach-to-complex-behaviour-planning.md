...
tldr: 针对动态环境中机器人行为规划需兼顾反应性、可解释性和适应性的挑战，现有方法多基于行为树，而有限状态机虽高效但缺乏生成式能力。本文提出生成式部分指定有限状态机(GPSFSM)神经符号架构，通过Fabric引擎解析执行含顺序、恢复、并行等控制结构的行为计划，并利用PromptTools统一接口组合局部与云端大语言模型。实验表明，在导航任务中GPSFSM计划生成成功率优于最新BTGenBot系统，零样本场景优势显著，且延迟与前沿LLM相当。工作首次将生成式FSM用于机器人行为规划，开源ROS2堆栈增强了方法的实用性和可复现性。
source: arxiv
...
motivation: 有限状态机虽在机器人中广泛采用且计算高效，但缺乏生成式方法，无法像行为树那样动态规划。
method: 提出GPSFSM神经符号架构，包含Fabric FSM引擎（支持四种控制结构）和异步事件系统，通过PromptTools统一调用本地/云端LLM。
result: 导航实验显示GPSFSM计划生成成功率高于BTGenBot，零样本场景表现更优，延迟与前沿模型相当。
conclusion: 首次实现生成式FSM规划，开源ROS2堆栈使该方法在机器人领域实用可复现。
---

## 摘要
在动态环境中运行的自主机器人需要结合反应性、可解释性和适应性的行为规划系统。尽管大型语言模型已成功与行为树集成用于动态重规划，但有限状态机尽管被广泛采用且计算效率高，却仍未被探索用于生成式方法。我们提出了一种生成式部分指定有限状态机（GPSFSM）神经符号架构，利用FSM的符号和语义结构（包括状态和事件触发的转换）来实现行为规划。本文介绍了首个用于机器人学的GPSFSM框架，其中包含Fabric——一个解析、验证并执行包含顺序、恢复、并行任意和并行全部控制结构的行为规划的FSM引擎。我们在ROS2中扩展了Capabilities2包，增加了异步事件系统用于行为链接，以及运行时参数注入用于可配置执行，解决了限制当前生成系统的临时函数表示问题。PromptTools为本地和云LLM提供了统一的ROS 2接口，带有提示缓冲，支持任务和上下文信息的动态异步组合。这些组件共同实现了与机器人无关的标准化语义能力描述。在导航任务上的实验评估表明，我们的GPSFSM方法在规划生成成功率上持续高于最先进的BTGenBot系统，尤其在零样本场景中表现优异（BT通常难以处理此类场景），同时保持与前沿LLM相当或更低的规划延迟。我们还展示了系统能够生成复杂行为。我们发布了开源的ROS2堆栈，使生成式FSM规划对于机器人系统既实用又可复现。
